...
def register(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            mail_subject = "Please activate your account of ShoeHolic"
            ...
            send_email.send()
            messages.success(request, 'Check Your Email and verify by clicking on the Link')
...
def forgotPassword(request):
    if request.method == 'POST':
        ...
        if Account.objects.filter(email=email).exists():
            ...
            current_site = get_current_site(request)
            mail_subject = "Reset Password of your ShoeHolic Account"
            ...
            messages.error(request, "Email Address Not Registered, Account Not Found !")
            return redirect("forgotPassword")
    return render(request, 'accounts/forgotPassword.html')



def resertpassword_validate(request, uidb64, token):
    try: 
        uid = urlsafe_base64_decode(uidb64).decode()
        user = Account._default_manager.get(pk=uid)
    except(TypeError, ValueError, OverflowError, Account.DoesNotExist):
